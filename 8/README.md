In this pattern, we print a number triangle where each row contains numbers starting from 1 up to the row number.

1.Run an outer loop (i) from 1 to N for rows.
2.For each row, run an inner loop (j) from 1 to i.
3.Print each number j in the current row.
